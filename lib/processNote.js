...
function makeLine(transitionColor, entropy, sizevar){
  ...
  if(window.points.length === 0){
    ...
    linearray = lineHelper(x1, y1, x2, y2, entropy);
    ...
    if(entropy === 0){
      ...
      while(i < linearray.length){
        modline.push(linearray[i]);
        i += Math.floor(entropy*Math.random())+1;
      }
      ...
      while(j < modline.length-2){
        singleLine = lineHelper(modline[j][0], modline[j][1], modline[j+1][0], modline[j+1][1]);
        manyLines = manyLines.concat(singleLine);
        j += 1;
      }

      window.pathGenerator.drawLine(manyLines);
    }
  }
  window.points.push([x2,y2]);
}

function lineHelper(x1, y1, x2, y2, entropy = 1/3) {
  let linearray = [[x1, y1]];
  ...
  for(let j = 0; j < distance; j++){
    ...
    linearray.push([Math.floor(x1+(xincrement*j)), (Math.floor(y1+(yincrement*j)+(Math.random()*entropy*3*plusminus)))]);
  }

  linearray.push([x2, y2]);
