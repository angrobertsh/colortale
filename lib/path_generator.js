...
class PathGenerator {
  constructor(color = [0, 0, 0], size = 300, entropy = 0) {
  ...
  drawLine(lineArray){
    ...
    let moveOver = () => {
      ...
      i += Math.floor(this.entropy/35)+1;
      // i += 1;
    ...
    let interval = window.setInterval(moveOver, this.entropy*Math.random());
  }
  ...
  changeBackGround(e) {
    if (this.night && e.target.id !== "night" && window.confirm("Entering Seraphic Fate. All work will be lost.")) {
      this.night = false;
      this.clear();
    }
    if (e.target.id === "default") {
      ...
      if (!this.night && window.confirm("Entering Dreaded Radius. All work will be lost.")) {
        this.night = true;
